...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 = isNotEmpty . find (\x -> x>4) . accumulate . map (const 1) 
  where accumulate' _ Nil = Nil
        accumulate' t (x :. xs) = (x+t) :. (accumulate' (x+t) xs)
...
produce ::
  ...
  -> a
  -> List a
produce f x = x :. (map f (produce f x))
