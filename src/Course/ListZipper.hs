...
findLeft f (ListZipper (l:.ls) x r) =
  if f l then
    isZ (ListZipper ls l (x:.r))
  else
    findLeft f (ListZipper ls l (x:.r))
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight f (ListZipper _ _ Nil) =
  isNotZ
findRight f (ListZipper l x (r:.rs)) =
  if f r then
    isZ (ListZipper (x:.l) r rs)
  else
    findRight f (ListZipper (x:.l) r rs)
...
nth n z =
  case index z of
    i | i == n -> isZ z
      | i < n -> moveRightN (n - i) z
      | otherwise -> moveLeftN (i - n) z
...
instance Applicative ListZipper where
  pure x = let xs = repeat x
           in ListZipper xs x xs

  (<*>) (ListZipper fl fx fr) (ListZipper vl vx vr) =
    ListZipper
      (zipWith ($) fl vl)
      (fx vx)
      (zipWith ($) fr vr)
...
instance Extend ListZipper where
  (<<=) f z =
    ListZipper
      (unfoldr (\lz -> case moveLeft lz of
                         MLZ Empty -> Empty
                         MLZ (Full lz') -> Full (f lz', lz'))
               z)
      (f z)
      (unfoldr (\lz -> case moveRight lz of
                         MLZ Empty -> Empty
                         MLZ (Full lz') -> Full (f lz', lz'))
               z)
...
instance Traversable MaybeListZipper where
  traverse f (MLZ Empty) =
    pure isNotZ
  traverse f (MLZ (Full z)) =
    isZ <$> traverse f z
