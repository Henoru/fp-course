...
runState' ::
  ...
  -> s
  -> (a, s)
runState' (StateT f) s = case f s of
  ExactlyOne (a,s') -> (a,s')
...
distinct' ::
  ...
  List a
  -> List a
distinct' l =eval' (filtering ff l) S.empty
  where ff a = do
                  s <- getT 
                  if S.member a s then
                    pure False
                  else do
                    putT (S.insert a s)
                    pure True
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF l = evalT (filtering ff l) S.empty
  where ff a = do 
                  s <- getT
                  if a>100 then
                    StateT $ \s -> Empty
                  else if S.member a s then
                    pure False
                    else do
                      putT (S.insert a s)
                      pure True
...
instance Functor k => Functor (OptionalT k) where
  (<$>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (<$>) f (OptionalT koa) = OptionalT $ ((<$>) f) <$> koa
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (<*>) (OptionalT kf) (OptionalT ka) = OptionalT $ do
                                                      f <- kf
                                                      case f of 
                                                        Empty -> pure Empty
                                                        Full f' -> do
                                                          a <- ka
                                                          pure (f' <$> a)
-- ot = OptionalT (Empty :. Empty :. Nil) <*> OptionalT (Full 1 :. Full 2 :. Nil)
...
instance Applicative (Logger l) where
  ...
  (<*>) ::
    ...
    -> Logger l a
    -> Logger l b
  (<*>) (Logger l0 f) (Logger l1 a) = Logger (l0 ++ l1) (f a)
...
distinctG l = runOptionalT $ evalT (filtering ff l) S.empty
  where ff :: (Integral a, Show a) =>
              a -> StateT (S.Set a) (OptionalT (Logger Chars)) Bool
        ff a =if a > 100 then
            constructStateT (OptionalT $ log1 (listh $ "aborting > 100: " P.++ (show a)) Empty)
            else do
              s <- getT
              let consLogB = if even a then 
                              log1 (listh $ ("even number: " P.++ (show a)))
                             else 
                              Logger Nil
              if S.member a s then
                constructStateT $ OptionalT $ consLogB (pure False)
              else do
                putT (S.insert a s)
                constructStateT $ OptionalT $ consLogB (pure True)

constructStateT :: Functor k => k a -> StateT s k a
constructStateT a = StateT $ \s -> ((\a -> (a,s)) <$> a)
